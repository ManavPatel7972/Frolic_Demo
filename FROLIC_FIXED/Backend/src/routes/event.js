...
router.get("/", async (req, res) => {
    try {
        const events = await Event.find().populate("DepartmentID", "DepartmentName");
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const event = await Event.findById(req.params.id).populate("DepartmentID", "DepartmentName");
...
router.delete("/:id", authMiddleware, async (req, res) => {
    try {
        ...
        await EventWiseWinners.deleteMany({ EventID: id });

        // 2. Delete associated Groups
        const groups = await Group.find({ EventID: id });
        const groupIds = groups.map(g => g._id);

        // 3. Delete Participants in those groups
        const Participant = require("../models/participant");
        await Participant.deleteMany({ GroupID: { $in: groupIds } });

        // 4. Delete the Groups
        await Group.deleteMany({ EventID: id });
